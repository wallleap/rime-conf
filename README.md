...
当输入长句的时候，可能会因为开启了辅助码，导致某些字输入的不正确（把下一个字的双拼识别成前个字的双形了），可以使用 `!` 手动分割

例如：`皮肤问题` 我只想输入前三个字（举个例子，正常不会这样输入，暂时想不出了，但实际输入长句是真的遇到过）

直接输入 `pifuwf` 会被识别成 `pi'fuwf`，导致输出 `皮玞`，此时可以输入 `pifu!wf` 来手动分割，正确输出 `皮肤问`

还有可以在中间输入 `!` 分隔，以便输入单字的辅助码，例如 `这一刻我感觉无比的自由` 假设当前模型会把 `无` 识别成 `五`，那可以在 `无` 前后输入 `!`，`!wuay!` 这样就可以在任意位置使用辅助码强制定字了
...
- `;` 选择第二个词
- `'` 选择第三个词（其实最好的是把这个当成分割符的，但是因为习惯了所以就只能替换成 `!` 了）
- `,` 选择第四个词
